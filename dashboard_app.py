...
state = {
    ...
    "calendar": None # Starts as None to indicate "Not Loaded"
}
...
class MoodEngine:
    ...
    def calculate(self, temp, brightness, faces, volume):
        ...
        avg_vol = sum(self.vol_history) / len(self.vol_history) if self.vol_history else 0

        eng_sound = (avg_vol / 100.0) * 0.7
        eng_light = (brightness / 255.0) * 0.3
        eng_social = 0.1 if faces > 0 else 0.0
        self.energy = max(0.0, min(1.0, eng_sound + eng_light + eng_social))

        val = 0.5
        dist_temp = abs(temp - 22.0)
        if dist_temp > 4: val -= 0.2
        else: val += 0.1
            
        if brightness > 100: val += 0.2
        elif brightness < 40: val -= 0.3 
            
        if faces > 0: val += 0.2
        if avg_vol > 75: val -= 0.4 
            
        self.valence = max(0.0, min(1.0, val))
        self._determine_label()
        return self.valence, self.energy

    def _determine_label(self):
        if self.valence >= 0.5:
            self.current_mood = "Energetic" if self.energy >= 0.5 else "Relaxed"
        else:
            self.current_mood = "Chaotic" if self.energy >= 0.5 else "Melancholy"
...
def on_message(client, userdata, msg):
    ...
    try:
        if msg.topic == config.TOPIC_CAMERA:
            ...
            data = json.loads(msg.payload.decode())
            # Double check user matches to prevent race conditions
            if data.get("user") == state["user"]:
                print(f"Received Calendar for {state['user']}")
            ...
            if msg.topic == config.TOPIC_SENSORS:
                ...
                # FIX: Clear calendar immediately if user changes
                new_user = payload.get("user", "Unknown")
                if new_user != state["user"]:
                    print(f"User Changed: {state['user']} -> {new_user}")
                    state["calendar"] = None # Instant wipe to Loading State
                state["user"] = new_user
                
            elif msg.topic == config.TOPIC_AUDIO:
                state["volume"] = int(payload.get("volume", 0))
    except Exception as e:
        print(f"MQTT Message Error ({msg.topic}): {e}")
...
class DashboardApp:
    def __init__(self, root, client):
        ...
        self.txt_sensors = self.canvas.create_text(self.w - 120, 40, text="--.- C | ---- mm", font=("Helvetica", 16), fill="#aaaaaa", anchor="e")
        ...
        self.cam_placeholder = ImageTk.PhotoImage(Image.new('RGB', (self.cam_w, self.cam_h), color='#111111'))
        self.cam_id = self.canvas.create_image(self.w - 10, self.h - 10, image=self.cam_placeholder, anchor="se")

        bar_x = 20
        bar_y = self.h - 30
        self.bar_bg_id = self.canvas.create_rectangle(self.cx - 100, bar_y, self.cx + 100, bar_y + 10, fill="#333333", outline="")
        self.bar_vol_id = self.canvas.create_rectangle(self.cx - 100, bar_y, self.cx - 100, bar_y + 10, fill="#00FF00", outline="")
        self.txt_debug = self.canvas.create_text(self.cx, self.h - 40, text="Vol", font=("Helvetica", 8), fill="gray", anchor="s")
    ...
    def update_ui(self):
        ...
        self.canvas.itemconfig(self.txt_sensors, text=f"{state['temp']}°C | {state['dist']}mm")
        ...
        if is_present:
            ...
            if state["user"] != "Unknown":
                ...
                cal_data = state["calendar"]
                # Handle Loading State (None) vs Empty State ([])
                if cal_data is None:
                    events = ["Loading Calendar..."]
                elif not cal_data:
            ...
            # IDLE MODE
            self.canvas.itemconfig(self.cam_id, image=self.cam_placeholder) # Clear Cam
